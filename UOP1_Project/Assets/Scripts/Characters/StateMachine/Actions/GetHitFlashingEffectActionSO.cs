...
[CreateAssetMenu(
    fileName = "GetHitFlashingEffectAction",
    menuName = "State Machines/Actions/Get Hit Flashing Effect"
)]
...
public class GetHitFlashingEffectAction : StateAction
{
    ...
    public override void Awake(StateMachine stateMachine)
    {
        ...
        _material = attackableEntity.MainMeshRenderer.materials[
            attackableEntity.MainMeshRenderer.materials.Length - 1
        ];
    ...
    private Color computeGetHitTintingColor()
    {
        ...
        float tintingTiming =
            (_getHitFlashingDuration - _innerFlashingTime)
            * _getHitFlashingSpeed
            / _getHitFlashingDuration;
        return Color.Lerp(
            _baseTintColor,
            finalTintingColor,
            (-Mathf.Cos(Mathf.PI * 2 * tintingTiming) + 1) / 2
        );
    }
}
